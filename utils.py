...
async def get_ha_state(entity_id: str = "") -> dict:
    ...
    print(
        f"Fetching HA state for {entity_id} from {HA_URL} with token {HA_TOKEN[:8]}..."
    )
...
def get_ratio_from_path(photo_path: str) -> float:
    ...
    if photo_path.endswith(".mp4") or photo_path.endswith(".avi"):
        ...
        if not ret:
            raise HTTPException(
                status_code=500,
                detail="Failed to read video frame for ratio calculation",
            )
    else:
...
async def get_random_photo_from_folder(
    folder_path: str, ratio: float | None = None, ratio_tolerance: float = 0.1
) -> FileResponse:
    ...
    if ratio is not None:
        ...
        photos = (
            in_tolerance if len(in_tolerance) >= 10 else [p for _, p in scored[:10]]
        )
    ...
    return FileResponse(
        selected_photo, media_type=media_type, headers={"cache-control": "no-cache"}
    )
...
def extract_random_frame(video_path: str) -> FileResponse:
    ...
    cap.release()
    return FileResponse(
        temp_path, media_type="image/jpeg", headers={"cache-control": "no-cache"}
    )
